...
## One Method: Intuitive Selection of Controls
...
- By using all such border pairs, they have much more statistical power than just a single comparison

. . .

::: {.callout-note}
## Key Idea
The border discontinuity design relies on institutional knowledge: counties right next to each other share local economic conditions, so the one that doesn't get the policy change is a natural control for the one that does.
:::


## Second Method: Data-Driven Selection of Controls
...
ggplot(plot_data_naive, aes(x = year, y = mean_y, color = Group, linetype = Group)) +
  ...
  labs(x = "Year", y = "Mean Uninsured Rate (%)",
    ...
    subtitle = "Pre-trends are not parallel: control states are declining more slowly")
```

::: aside
Note: figure generated using simulated data.
:::


## Choosing Better Control States
...
- Why?
  - States which were highly likely to expand but didn't are much more *similar* to states which did expand
...
- The *predicted probability of being in the treatment group*, $\hat{P_i}$, is the **propensity score**
...
- In other words:
  - Untreated units that *look like* they should have been treated get **a lot of weight**
  - Untreated units that look *nothing like* treated units get **very little weight**
...
- But the **true effect** is $-5$! The estimate is roughly *twice as large* as it should be.
...
- We need treated and control propensity scores to **overlap** --- otherwise there's no good comparison
...
- The $\hat{\beta_k}$ coefficients trace out the effect of the policy over time *relative* to $k=−1$
...
- Post-treatment coefficients (`rel_t_treat_0` through `rel_t_treat_5`) show the effect **grows** over time:
  - From $\approx -100$ in the year of treatment to $\approx -500$ five years after
- Pre-treatment coefficients (`rel_t_treat_-10` through `rel_t_treat_-2`) are mostly **small and statistically insignificant**
...
- Errors are *correlated within the cluster*

- Errors are *independent between clusters*
...
## What Happens When Standard Errors Are Too Small?
...
- In the ACA expansion, the policy is set by the state, so we cluster at the state level:
...
- It is up to the researcher to define the clusters
...
- When in doubt, cluster at a broader level (e.g., state rather than county) --- this is conservative
...
## Summary: What We Learned Today {.smaller}

1. **Choosing good controls** is the foundation of any DiD analysis
    - Use institutional knowledge (border designs) or data-driven methods (IPW)

2. **IPW-DiD** reweights controls so that the control group resembles the treated group on observables
    - Propensity scores $\rightarrow$ IPW weights $\rightarrow$ weighted TWFE

3. **Dynamic DiD** estimates treatment effects that change over time
    - Pre-treatment coefficients = test for parallel trends
    - Post-treatment coefficients = causal effects unfolding over time
    - Present with event study plots

4. **Cluster-robust SEs** fix inflated Type I error rates from correlated errors in panel data
