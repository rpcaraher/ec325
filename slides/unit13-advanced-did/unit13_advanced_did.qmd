...
- Alternatively, we can create a **factor** variable which equals the *relative time* for treated units and 0 for control units
...
did_ex_tab <- did_ex_tab |>
  mutate(rel_treat_t = case_when(treatment_group == 1 ~ rel_t,
    TRUE ~ 0),
    rel_treat_t.f = as.factor(rel_treat_t),
    rel_t_treat.f = relevel(rel_treat_t.f, ref = "-1")
    )

as.data.frame(
  did_ex_tab |> select(unit_id, year, treatment_group, rel_t_treat.f)
)
...
final_simulated_data <- panel_env |>
  ...
  mutate(
    city_fe = city_fes[city_id] + treated_group * group_level_gap,
...
final_simulated_data <- dummy_cols(final_simulated_data,
  select_columns = "rel_t_treat", remove_selected_columns = FALSE)
...
final_simulated_data <- final_simulated_data |>
  mutate(rel_t_treat.f = as.factor(rel_t_treat),
    rel_t_treat.f = relevel(rel_t_treat.f, ref = "-1"))
...
- Instead estimating using the factor variable:
...
dyn_did_2 <- feols(manufacturing_emp ~ rel_t_treat.f |
    city_id + year,
    vcov = "HC1",
    data = final_simulated_data)
