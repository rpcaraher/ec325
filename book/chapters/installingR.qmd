...
The `glimpse()` function from the tidyverse shows you the type of every column in a data frame:
...
Each variable is shown with its type abbreviation (`<dbl>` for double, `<int>` for integer, etc.).
This is one of the first things you should run when loading a new dataset.
