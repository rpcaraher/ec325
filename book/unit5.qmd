...
::: {.callout-note collapse="true"}
## Exercise 1: Interpreting t-Statistics

Consider a regression where $\hat{\beta}_1 = 2.5$ and $se(\hat{\beta}_1) = 0.8$.

a) Calculate the t-statistic for testing $H_0: \beta_1 = 0$.
b) Is this significant at the 5% level? The 1% level?
c) Calculate the 95% confidence interval for $\beta_1$.
d) If someone claimed that $\beta_1 = 4$, would the 95% CI support or contradict this claim?
...
For each p-value below, state: (i) whether you reject $H_0$ at the 10%, 5%, and 1% levels, and (ii) interpret what the p-value means.
...
::: {.callout-note collapse="true"}
## Exercise 3: F-Test Application

You estimate a model of house prices with the following variables: lot area, garage size, bedrooms, bathrooms, year built, and neighborhood fixed effects (20 neighborhood dummies).

a) How would you test whether neighborhood matters for house prices?
b) What is the null hypothesis?
c) How many restrictions (q) would you be testing?
d) If the F-statistic is 3.45 and the critical value is 1.57, what do you conclude?

:::


::: {.callout-note collapse="true"}
## Exercise 4: Reading Regression Tables

Find a regression table in a published economics paper (or use one provided by your instructor).

a) Identify the dependent and independent variables.
b) What is the key coefficient of interest?
c) Is it statistically significant? At what level?
d) How does the coefficient change across different specifications?
e) Is the effect practically significant? Explain your reasoning.
...
Using R, replicate the sampling distribution simulation from this chapter, but with $\beta_1 = 0.5$ instead of $\beta_1 = 0$. 

a) Generate the sampling distribution using 1,000 simulated samples.
b) What is the average of your estimates? How does it compare to the true $\beta_1$?
c) Create a histogram and overlay the theoretical normal distribution.
d) Calculate how often your 95% confidence intervals contain the true value.
